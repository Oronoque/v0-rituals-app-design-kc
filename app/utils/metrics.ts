...
export const generateMockMetrics = (
  ...
  const completionRate = Math.round(
    (history.filter((h) => h.completed).length / history.length) * 100
  );
  ...
  if (stepType === "weightlifting") {
    ...
    baseMetrics.personalBest = Math.max(
      ...fitnessData.personalRecords.map((pr) => pr.oneRepMax)
    );
  }
...
export const generateRitualMetricsData = (ritual: Ritual) => ({
  summary: {
    totalCompletions:
      ritual.metrics?.totalCompletions || Math.floor(Math.random() * 50) + 20,
    completionRate:
      ritual.metrics?.completionRate || Math.floor(Math.random() * 40) + 60,
    currentStreak:
      ritual.metrics?.currentStreak || Math.floor(Math.random() * 15) + 1,
    longestStreak:
      ritual.metrics?.longestStreak || Math.floor(Math.random() * 25) + 5,
    trend: "up" as const,
...
export const generateStepMetricsData = (step: Step) => {
  const baseData = {
    summary: {
      totalCompletions:
        step.metrics?.totalCompletions || Math.floor(Math.random() * 40) + 15,
      completionRate:
        step.metrics?.completionRate || Math.floor(Math.random() * 35) + 65,
      currentStreak:
        step.metrics?.currentStreak || Math.floor(Math.random() * 10) + 1,
      longestStreak:
        step.metrics?.longestStreak || Math.floor(Math.random() * 20) + 3,
      personalBest:
        step.type === "weightlifting"
