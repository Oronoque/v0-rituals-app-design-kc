...
export function ChangelogScreen({
  ...
  const [activeTab, setActiveTab] = useState<
    "timeline" | "forks" | "creators" | "rewards"
  >("timeline");
  ...
  const mockChangelog: ChangelogEntry[] = [
    {
      ...
      description:
        "Initial creation of comprehensive morning routine for peak performance",
    ...
    },
    {
      ...
      description:
        "Adapted the productivity stack for college students with limited time",
      author: "student_life",
      authorAvatar: "SL",
    ...
    },
    {
      ...
      description:
        "Added breathing exercises and gratitude practice based on user feedback",
    ...
    },
    {
      ...
      description:
        "Specialized version for athletes with pre-training preparation",
    ...
    },
    {
      ...
      description:
        "Integrated time estimates for each step to improve planning",
      author: "time_master",
      authorAvatar: "TM",
  ...
  const filteredChangelog = selectedRitual
    ? mockChangelog.filter(
        (entry) =>
          entry.ritualId === selectedRitual || entry.parentId === selectedRitual
      )
    : mockChangelog;
    ...
    <div className="space-y-4">
      ...
      {showGlobal && (
        <div className="ios-card p-4">
          <div className="text-white font-medium text-ios-subhead mb-3">
            Filter by Ritual
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            {Array.from(
              new Set(mockChangelog.map((entry) => entry.ritualId))
            ).map((ritualId) => {
              const ritual = mockChangelog.find(
                (entry) => entry.ritualId === ritualId
              );
              return (
                <Button
                  key={ritualId}
      ...
      <div className="space-y-3">
        {filteredChangelog.map((entry, index) => (
          <div
            key={entry.id}
            className={cn("ios-card p-4 border", getChangeColor(entry.type))}
          >
            {/* Header */}
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <span className="text-white font-medium text-ios-subhead">
                      {entry.title}
                    </span>
                  </div>
                  ...
                  {entry.parentAuthor && (
                    <div className="text-[#AEAEB2] text-ios-footnote">
                      forked from {entry.parentAuthor}'s version
                    </div>
                  )}
                </div>
              </div>
                ...
                {entry.metrics && (
                  <div className="flex items-center space-x-3 mt-1">
                    <div className="flex items-center space-x-1">
                      ...
                      <span className="text-ios-footnote text-blue-400">
                        {entry.metrics.adoptions}
                      </span>
                    </div>
                    <div className="flex items-center space-x-1">
                      <Star className="w-3 h-3 text-yellow-400" />
                      <span className="text-ios-footnote text-yellow-400">
            ...
            {/* Description */}
            <p className="text-white text-ios-subhead mb-3">
              {entry.description}
            </p>
                ...
                {entry.changes.added && entry.changes.added.length > 0 && (
                  <div>
                    <div className="flex items-center space-x-2 mb-1">
                      ...
                      <span className="text-ios-footnote text-green-400 font-medium">
                        Added
                      </span>
                    </div>
                    <div className="ml-5 space-y-1">
                      {entry.changes.added.map((item, i) => (
                        <div
                          key={i}
                          className="text-ios-footnote text-[#AEAEB2]"
                        >
                          • {item}
                        </div>
                      ))}
                ...
                {entry.changes.removed && entry.changes.removed.length > 0 && (
                  <div>
                    <div className="flex items-center space-x-2 mb-1">
                      ...
                      <span className="text-ios-footnote text-red-400 font-medium">
                        Removed
                      </span>
                    </div>
                    <div className="ml-5 space-y-1">
                  ...
                  entry.changes.modified.length > 0 && (
                    <div>
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        <span className="text-ios-footnote text-orange-400 font-medium">
                          Modified
                        </span>
                      </div>
                      <div className="ml-5 space-y-1">
  ...
  const renderForks = () => {
    const forkEntries = filteredChangelog.filter(
      (entry) => entry.type === "fork" || entry.type === "improvement"
    );

    return (
      <div className="space-y-4">
        <div className="ios-card p-4">
          <h3 className="text-white font-medium text-ios-subhead mb-3">
            Fork Network
          </h3>
          <div className="text-[#AEAEB2] text-ios-footnote">
            Showing {forkEntries.length} forks and improvements
          </div>
        </div>
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1">
                    ...
                    {entry.parentAuthor && (
                      <div className="text-blue-400 text-ios-footnote">
                        ↳ forked from {entry.parentAuthor}
                      </div>
                    )}
                  </div>
                </div>
  ...
  const renderCreators = () => (
    <div className="space-y-4">
      <div className="ios-card p-4">
        ...
        <div className="text-[#AEAEB2] text-ios-footnote">
          Ranked by community impact and adoption
        </div>
      </div>
          ...
          <div key={creator.author} className="ios-card p-4">
            ...
            <div className="grid grid-cols-3 gap-3 mb-3">
              <div className="text-center">
                <div className="text-ios-subhead font-bold text-white">
                  {creator.totalAdoptions}
                </div>
                <div className="text-ios-footnote text-[#AEAEB2]">
                  Adoptions
                </div>
            ...
            <div className="flex flex-wrap gap-2">
              {creator.badges.map((badge, i) => (
                <span
                  key={i}
                  className="px-2 py-1 bg-blue-500/20 text-blue-400 text-ios-footnote rounded-full"
                >
                  {badge}
                </span>
              ))}
      ...
      <div className="ios-card p-4">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
            ...
            <div className="text-yellow-400 text-ios-footnote">
              Premium features
            </div>
          </div>
          <div className="flex items-center justify-between p-3 bg-gray-500/10 border border-gray-500/30 rounded-lg">
            ...
            <div className="text-gray-400 text-ios-footnote">
              Special badges
            </div>
          </div>
